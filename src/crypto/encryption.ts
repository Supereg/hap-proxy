...
export function layerEncrypt(data: Buffer, context: HAPEncryptionContext) {
  ...
  for (let offset = 0; offset < total; ) {
    ...
    const encrypted = chacha20_poly1305_encryptAndSeal(context.encryptionKey, nonce, leLength, data.slice(offset, offset + length))
    offset += length;
...
export function layerDecrypt(packet: Buffer, context: HAPEncryptionContext) {
  ...
  for (let offset = 0; offset < total;) {
    ...
    const plaintext = chacha20_poly1305_decryptAndVerify(context.decryptionKey, nonce, packet.slice(offset,offset+2), packet.slice(offset + 2, offset + 2 + realDataLength), packet.slice(offset + 2 + realDataLength, offset + 2 + realDataLength + 16));
