...
export function chacha20_update(ctx: Chacha20Ctx, dst: Buffer, src: Buffer, inlen: number) {
    ...
    if (inlen > 0) {
        if (src.length > 0) {
            ...
            const zeros = Buffer.alloc(inlen);
...
export function aead_mac(key: Buffer, ciphertext: string, data: string) {
    ...
    const m: any = new Array(clen + dlen + 16);
    let i = dlen;
...
export function aead_decrypt(ctx: AeadCtx, nonce: Buffer, ciphertext: any, ad: string) {
    const c = new Chacha20Ctx();
    const key = aead_init(c, ctx.key, nonce);
    const clen = ciphertext.length - Poly1305TagSize;
    const digest = ciphertext.slice(clen);
    const mac = aead_mac(key, ciphertext.slice(0, clen), ad);
